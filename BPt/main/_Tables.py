...
from ..helpers.Table_Helpers import (get_split_df, get_base_title,
                                     filter_out_nans,
                                     proc_group_by, get_subjects_by_group,
                                     save_table)
...
def Save_Table(self, save_loc, targets='SHOW_ALL',
    ...
    Note: you must have the optional library python-docx
    ...
    group_by : str or list, optional
        ...
        different binary / categorical (or multilabel)
        groups. For example, by passing 'split',
        (assuming the split param has been left as True),
        ...
        If a list of values is passed, then each element
        will be used for its own
        seperate split. Further, if the `include_all` parameter
        is left as its default
        value of True, then in addition to a single or
        multiple group_by splits,
        the values over all subjects will also be displayed.
        (E.g. in the train test
        split case, statistics would be shown for the full sample,
        only the train subjects
        and only the test subjects).

        (default = None)

    split : bool, optional
        If True, then information about the global train test split
        will be added as a binary feature of the table.
        If False, or if a global split has not
        yet been defined, this split will not be added to the table. Note that
        'split' can also be passed to `group_by`.

        Note: If it is desired to create a table with
        just the train subjects (or test)
        for example, then the `subjects` parameter should be used.
        If subjects is set
        to 'train' or 'test', this `split` param will
        switch to False regardless of
        user passed input.
    ...
    include_all : bool, optional
        ...
        param(s) are passed, then `include_all` will
        be True regardless of user passed value.
    ...
    subjects : None, 'train', 'test' or array-like, optional
        ...
        Alternatively this parameter can be used to pass
        just the train_subjects
        with 'train' or just the test subjects with 'test'
        or even a custom
    ...
    style : str or None, optional
        The default .docx table style in which to use, which contrals
        things like
        table color and fonts, ect... Keep style as None by default to just
        use the default style, or feel free to try passing any of a
        huge number of
        preset styles. These styles can be found at the bottom of
        https://python-docx.readthedocs.io/en/latest/user/styles-understanding.html
        under "Table styles in default template".

        Some examples are: 'Colorful Grid', 'Dark List', 'Light List',
        'Medium List 2', 'Medium Shading 2 Accent 6', ect...
    ...
    for grp in range(len(gb_names)):
        subjects_by_group =\
            get_subjects_by_group(gb_dfs[grp], gb_encoders[grp], gb_names[grp])

        for subjects in subjects_by_group:
            indv_titles, values =\
                self._get_table_contents(
                    dfs, encoders, names,
                    subjects=subjects,
...
def _get_single_dfs(self, targets='SHOW_ALL',
                    covars='SHOW_ALL',
                    strat='SHOW_ALL', split=True,
    ...
    all_encoders =\
        [self.targets_encoders, self.covars_encoders, self.strat_encoders]

    return_dfs = []
    return_encoders = []
    return_names = []

    for names, class_df, func, encoders in zip(all_names,
                                               class_dfs,
                                               funcs, all_encoders):

        if (names is not None) and (len(class_df) > 0):
            df = self._get_plot_df(class_df, subjects, show_only_overlap)

            names = func(names)
            for name in names:
                single_df, encoder = self._get_single_df(name, df, encoders)
    ...
    if split and self.train_subjects is not None:
        ...
        split_df = get_split_df(
            self.subject_id, self.train_subjects, self.test_subjects)
...
def _get_table_contents(self, dfs, encoders, names,
    ...
    for df, encoder, name in zip(dfs, encoders, names):
        ...
        no_nan_subjects, no_nan_data, nan_subjects = filter_out_nans(df)

        if subjects is not None:

            # Set no_nan_data to only overlap of subjects passed
            overlap_no_nan_subjects = np.intersect1d(no_nan_subjects, subjects)
            no_nan_data = no_nan_data.loc[overlap_no_nan_subjects]

            # Update number of nan subjects to reflect subject subset
            nan_subjects = np.intersect1d(nan_subjects, subjects)

        print('Number of dropped subjects:', len(nan_subjects))
        ...
        if encoder is None:
            ...
            value = str(np.round(np.mean(no_nan_data[name]), rnd_to))
            rnded = str(np.round(np.std(no_nan_data[name]), rnd_to))
            value += ' (±' + rnded + ')'
        ...
        # Binary/ordinal or one-hot
            ...
            display_df, _, _ = self._get_cat_display_df(no_nan_data, encoder,
                                                        name,
...
def _get_group_titles(self, gb_dfs, gb_encoders,
                      gb_names, cat_show_original_name):
